...
ndvCalc <- function(x) {
  if (x=='wageCloudFree1') {
  ndvi <- (x[[4]] - x[[3]]) / (x[[4]] + x[[3]]) #bands for Landsat 8 and 5 are different
    return(ndvi)
  } else {
...
ndvCalc (wageCloudFree1)
ndvi1 <- calc(x=wageCloudFree1, fun=ndvCalc) #insert subset/bands after stacking 
ndvi2 <- overlay(x=wageCloudFree2, fun=ndvCalc) #insert subset/bands after stacking 
plot(ndvi2)
#check all.equal
